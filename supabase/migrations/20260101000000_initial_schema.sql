...
-- Converts the base schema (supabase/schema.sql) into a migration so that
-- Supabase preview branches create these tables before subsequent migrations run.
-- All statements are idempotent for safety on existing databases.
...
drop policy if exists "Users can view own metrics" on public.body_metrics;
create policy "Users can view own metrics"
  on public.body_metrics for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert own metrics" on public.body_metrics;
create policy "Users can insert own metrics"
  on public.body_metrics for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own metrics" on public.body_metrics;
create policy "Users can update own metrics"
  on public.body_metrics for update
  using (auth.uid() = user_id);

-- Trainer invites policies
drop policy if exists "Trainers can manage own invites" on public.trainer_invites;
create policy "Trainers can manage own invites"
  on public.trainer_invites for all
  using (auth.uid() = trainer_id);

drop policy if exists "Anyone can read invite by code" on public.trainer_invites;
...
drop policy if exists "Clients can view own trainer" on public.trainer_clients;
create policy "Clients can view own trainer"
  on public.trainer_clients for select
  using (auth.uid() = client_id);

drop policy if exists "System can insert trainer_clients" on public.trainer_clients;
create policy "System can insert trainer_clients"
  on public.trainer_clients for insert
  with check (true);

drop policy if exists "Trainer can update own client relationships" on public.trainer_clients;
create policy "Trainer can update own client relationships"
  on public.trainer_clients for update
  using (auth.uid() = trainer_id);

drop policy if exists "Client can update own trainer relationship" on public.trainer_clients;
create policy "Client can update own trainer relationship"
  on public.trainer_clients for update
  using (auth.uid() = client_id);

-- Custom workouts policies
drop policy if exists "Creators can manage own custom workouts" on public.custom_workouts;
create policy "Creators can manage own custom workouts"
  on public.custom_workouts for all
  using (auth.uid() = creator_id);

drop policy if exists "Public workouts are readable by all" on public.custom_workouts;
